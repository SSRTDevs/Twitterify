...
export default function RightThread(props) {
    return (
        <>
            ...
            <div className="overflow-y-scroll h-2/3 w-full checker-bg border rounded border-gray-200">
                <div className="w-full">
                ...
                <div className="reply-div chat chat-end">
                    {
                        Object.keys(props.thread.details).length === 0 ?
                            ...
                            props.thread.details["reply_tweets"].map((item, key) => {
                                return <div className="chat-bubble replies text-md border border-x-0 border-gray-700 hover:bg-[#f4f4f40f]">{item}</div>
                            })
