  ...
  return (
    <>
      <div className="tooltip tooltip-top p-4" data-tip={emotion}>
        {emotion === "anger" && (
          <ImAngry className={`text-red-500 ${className}`} size={30} />
        )}
        {emotion === "fear" && (
          <BsEmojiFrown className={`text-yellow-500 ${className}`} size={30} />
        )}
        {emotion === "neutral" && (
          <BsEmojiNeutral
        ...
        {emotion === "joy" && (
          <BiHappy className={`text-green-500 ${className}`} size={35} />
        )}
        {emotion === "surprise" && (
      ...
      <div className="container mx-auto w-[75%] h-1/10">
        <div>
          <Breakpoint small down>
            <InputWithBtn
              run={() => thread_summarizer(thread, setThread)}
