...
function TrendingCard({ index, trend, hashtag, setReadTweets, RightComponent }) {
    return (
        <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div
                    id={`${hashtag}`}
                    className="card shadow-md rounded !bg-[#1c1c1c] overflow-x-hidden"
                    ...
                    <div className="card-body p-5 space-y-1">
                        <div className="topbar w-full flex justify-between items-center">
                            ...
                            <div className="flex justify-between gap-3 text-center text-xl cursor-auto p-2">
                                <div className="tooltip tooltip-left flex justify-between gap-2" data-tip="Positive">
                                    <span className="stat-figure text-green-600">
                                        <MdSentimentSatisfiedAlt size={25} />
                                    </span>
                                    {trend["pos"]}
                                </div>
                                <div className="tooltip tooltip-top flex justify-between gap-2" data-tip="Neutral">
                                    <span className="stat-figure text-yellow-600">
                                        <MdSentimentNeutral size={25} />
                                    </span>
                                    {trend["neu"]}
                                </div>
                                <div className="tooltip tooltip-right flex justify-between gap-2" data-tip="Negative">
