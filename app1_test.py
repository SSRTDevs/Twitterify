...
summ_tokenizer = AutoTokenizer.from_pretrained("/Users/shashwat/python_projects/twitter_2/twitter-apis/tokenzier")
summ_model = AutoModelForSeq2SeqLM.from_pretrained("/Users/shashwat/python_projects/twitter_2/twitter-apis/model")


sen_tokenizer = AutoTokenizer.from_pretrained("/Users/shashwat/python_projects/twitter_2/twitter-apis/tokenizer_sen")

sen_model = AutoModelForSequenceClassification.from_pretrained("/Users/shashwat/python_projects/twitter_2/twitter-apis/model_sen")
...
def tweet_summarizer(tweets, summarizer):
    try:
        res = summarizer(tweets)
        return res[0]["summary_text"]
    except:
        print("Sequence length too large for model, cutting text in half and calling again")
        return tweet_summarizer(tweets[:(len(tweets) // 2)], summarizer) + tweet_summarizer(tweets[(len(tweets) // 2):], summarizer)
    ...
    for locations in api.closest_trends(latitude, longitude):
        for trends in api.get_place_trends(locations['woeid']):
            ...
            for trend in trending_topics_volume:
                ...
                summary_result[q] = tweet_summarizer(" ".join(result[q]), summarizer)
...
sum_res, ana_res = trending_tweets(api, client, "Mumbai", 20, 4, summarizer)
